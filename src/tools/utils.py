...
def build_dataset(config, ues_word, n_samples=None, lang="en", sep="\t", save_vocab=False, use_saved_vocab=False,
                  multi_label=False):
    if ues_word:
        if lang == "cn":
    ...
    def load_dataset(path, pad_size=32):
        ...
        with open(path, 'r', encoding='UTF-8', newline='') as f:
            ...
            for line in tqdm(f):
                ...
                contents.append(
                    (words_line, int(label) if not multi_label else list(map(int, label.split(','))), seq_len))
        return contents  # [([...], 0), ([...], 1), ...]
...
def get_pretrained_embeddings(pretrained_name_or_path, vectors=None, vocab=None,
                              cache='./data/pretrained_models/.vector_cache'):
    if vectors is None:
        vectors = Vectors(name=pretrained_name_or_path, cache=cache)
    if vocab is None:
        return vectors.vectors
    return vectors.get_vecs_by_tokens(list(vocab.keys()))

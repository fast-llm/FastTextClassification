...
parser.add_argument('--dataset', type=str, default="dbpedia",
                    help='dataset name')
parser.add_argument('--save', type=str, default="dbpedia",
                    help='dataset name')
...
if __name__ == '__main__':
    embedding = 'embedding_SougouNews.npz' if args.lang == "cn" else "glove.840B.300d.txt"
    ...
    if args.architecture == "transformers":
        ...
        vocab, train_data, dev_data, test_data = build_dataset(config, args.word, n_samples=n_samples, lang=args.lang,
                                                               sep=sep)
        config.vocab = vocab
